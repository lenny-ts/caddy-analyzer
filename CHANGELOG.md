...
### Added
- **Automatic City GeoIP download**: download the GeoLite2 City database alongside Country and ASN data unless `--no-auto-download` is set.
...
### Fixed
- **Automatic cosign download**: use the current `cosign_checksums.txt` asset name when downloading cosign for signature verification.
...
- **`--trust-forwarded` ignored in `top` and `tail`**: forwarded client IPs are now applied before filtering and GeoIP enrichment in both subcommands. Closes #64.
